...
# CORRECTION: Use df_clean$GPA (not df$GPA)
if(!"GPA_Num" %in% names(df_clean)) {
  # This was the broken line. Now it is fixed:
  df_clean$GPA_Num <- as.numeric(df_clean$GPA)
}

ggplot(df_clean, aes(x = GPA_Num)) +
...
The distribution of academic performance exhibits a pronounced negative skew (left-skew), with the probability density peaking in the 'Good' (3.2–3.6) and 'Excellent' (\>3.6) categories. Unlike a standard normal distribution (Bell Curve), this pattern indicates that high achievement is the norm within this university cohort. However, the 'long left tail' extending into the 'Poor' and 'Average' ranges helps identify a specific minority of students (the at-risk tail) who are significantly detached from the general success trend. This suggests that university interventions should be targeted at this specific subgroup rather than applied broadly, as the majority of the population is already performing at a high standard.

```{r}
ggplot(df_clean, aes(x = GPA_Num, fill = Gender_Label)) +
  ...
  labs(
    title = "Performance Distribution by Gender and Socioeconomic Status",
    subtitle = "Comparing Density Curves across Household Types",
    x = "GPA Score",
    y = "Density",
    fill = "Gender"
  )
...
This faceted density plot compares the shape of performance for Male (Blue) and Female (Red) students, split by their Economic Status (Poor vs. Non-Poor).

-   **In Non-Poor Households:** The curves for males and females overlap significantly, though the female curve peaks slightly higher at the top end.

-   **In Poor Households:** I see a flatter distribution, indicating more variability. Crucially, the "Female" curve in the Poor group shifts slightly to the right of the "Male" curve, suggesting that female students may be more resilient to economic disadvantages in terms of maintaining grades compared to their male counterparts in similar economic conditions.

-   **The Economic Variance:** Students from 'Poor Households' display a flatter, wider distribution (higher variance) compared to the sharper peaks of the 'Non-Poor' group. This indicates that economic instability is associated with inconsistent academic outcomes—some poor students excel, while many others struggle.

-   **The Gender Resilience Gap:** Crucially, within the 'Poor Household' demographic, the female distribution (red curve) is shifted rightward relative to the male distribution (blue curve). This implies that female students demonstrate higher academic resilience—they are better able to maintain high performance despite economic disadvantages—whereas male students appear more vulnerable to the academic penalties of poverty."

```{r}
ggplot(df_clean, aes(x = Father_Edu_Label, y = GPA_Num, fill = Father_Edu_Label)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.1) +
  theme_bw() +
  scale_fill_brewer(palette = "Spectral") +
  labs(
    title = "Relationship between GPA and Father's Education",
...
The boxplot visualizes the intergenerational transmission of advantage. I observe a clear socioeconomic gradient: as the father's education level rises from 'Primary' to 'University', the median GPA increases stepwise.

-   **The 'Safety Net' Effect:** Beyond just the average score, looking at the *height* of the boxes (Interquartile Range) reveals that students with university-educated fathers have a much narrower distribution. This suggests that high parental education acts as a protective factor that reduces the *risk* of low performance, effectively creating a 'floor' below which these students rarely fall.

-   **High Volatility in Low Education:** Conversely, students from 'Primary' education backgrounds show high volatility (taller boxes), indicating that while high achievement is possible, the risk of falling into the lower GPA quartiles is significantly higher."
