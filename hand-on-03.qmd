...
```{r}
# 1. Load ALL required packages fresh
pacman::p_load(readxl, gifski, gapminder, plotly, gganimate, tidyverse, png)

# 2. Reload the data (GlobalPopulation)
col <- c("Country", "Continent")
globalPop <- read_xls("GlobalPopulation.xls", sheet="Data") %>%
  mutate(across(all_of(col), as.factor)) %>%
  mutate(Year = as.integer(Year))

# 3. Create the animation object (assign to 'p', don't print yet)
...
# 4. Render explicitly using gifski_renderer()
# This forces R to use the correct engine instead of guessing
...
gg <- ggplot(pop_data_simulated, 
  ...
  geom_point(aes(size = Total_Pop,
                 ...
                 frame = Time,   # Creates the animation frames

                 ids = PA),      # Ensures smooth transition of specific bubbles
